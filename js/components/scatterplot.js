...
define(['knockout', 'text!./scatterplot.html', 'jnj_chart'], function (ko, view, jnj_chart) {
	...
	function scatterplot(params) {
		var self = this;
		self.loading = ko.observable(true).extend({deferred:true});
		self.data = params.data;
		self.processedData = ko.observable().extend({deferred:true});
		self.chartResolution = ko.observable().extend({deferred:true});
		self.jsonFile = params.jsonFile;
		self.csvFile = params.csvFile;
		self.tsvFile = params.tsvFile;
		self.chartOptions = ko.observable(params.chartOptions).extend({deferred:true});
		...
		if (self.data) {
			...
			loadData(self.jsonFile, dataLoaded.bind(self), self.chartOptions());
		} else if (self.csvFile) {
			d3.csv(self.csvFile, dataLoaded.bind(self), self.chartOptions());
		} else if (self.tsvFile) {
			d3.tsv(self.tsvFile, dataLoaded.bind(self), self.chartOptions());
		} else {
			self.loading(false);
